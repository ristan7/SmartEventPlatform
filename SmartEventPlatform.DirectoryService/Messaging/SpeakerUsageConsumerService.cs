    ...
    public sealed class SpeakerUsageConsumerService : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            _logger.LogInformation(
                "SpeakerUsageConsumerService started. Queue='{Queue}', DLQ='{DLQ}', MaxRetries={Max}.",
                    ...
                    if (eventType == nameof(EventSpeakerAddedEvent))
                    {
                        ...
                        if (evt is not null)
                        {
                            ...
                            if (!exists)
                                ...
                                { EventSpeakerId = evt.EventSpeakerId, SpeakerId = evt.SpeakerId });
                        }
                        else
                    ...
                    else if (eventType == nameof(EventSpeakerRemovedEvent))
                    {
                        ...
                        if (evt is not null)
                        {
                            ...
                            if (tracker is not null)
                                ...
                                _logger.LogWarning("SpeakerUsageTracker not found. EventSpeakerId={Id}.", evt.EventSpeakerId);
                        }
                        else
                            _logger.LogWarning("Could not deserialize. MessageId={Id}, EventType={T}.", messageId, eventType);
                    }
